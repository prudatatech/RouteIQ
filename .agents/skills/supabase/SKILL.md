...
- **Auth and session security**
  ...
  - **Do not use database triggers on `auth.users` to automatically create profile rows in `public.users`**. These triggers often fail silently (e.g., returning 500s) causing the GoTrue auth transaction to rollback and blocking user signups. Instead, handle profile creation manually in the application code (backend or frontend) immediately after a successful authentication/signup event.
